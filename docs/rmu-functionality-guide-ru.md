...
Внутри MCP-сервера используется `rmu-core`.

## MCP

`rmu-mcp-server` нужен для MCP-клиентов и агентских сценариев.
...
- нужен быстрый ручной проход по проекту через MCP tool calls
