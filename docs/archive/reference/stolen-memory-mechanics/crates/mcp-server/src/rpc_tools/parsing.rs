...
pub(crate) fn parse_optional_usize_with_min(
    args: &Value,
    tool: &str,
    field: &str,
    minimum: usize,
...
pub(crate) fn parse_optional_bool(args: &Value, tool: &str, field: &str) -> Result<Option<bool>> {
    let Some(value) = args.get(field) else {
        return Ok(None);
    };
...
pub(crate) fn parse_required_non_empty_string(
    args: &Value,
    tool: &str,
    field: &str,
) -> Result<String> {
...
pub(crate) fn parse_required_string_list(
    args: &Value,
    tool: &str,
    field: &str,
) -> Result<Vec<String>> {
    ...
    let items = value
        .as_array()
        .ok_or_else(|| invalid_params_error(format!("{tool} requires array `{field}`")))?;
    ...
    for item in items {
        ...
        if trimmed.is_empty() {
            return Err(invalid_params_error(format!(
                "{tool} requires non-empty string items in `{field}`"
            )));
        }
...
pub(crate) fn parse_optional_string_list(
    args: &Value,
    tool: &str,
    field: &str,
...
pub(crate) fn reject_unknown_fields(args: &Value, tool: &str, allowed: &[&str]) -> Result<()> {
    let object = args
        .as_object()
