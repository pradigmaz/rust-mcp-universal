...
pub(super) fn validate_tools_call_params(
    ...
    match object.get("name") {
        ...
        _ => return Err("tools/call requires string field `name`".to_string()),
    }
...
pub(super) fn validate_initialize_params(
    ...
    if let Some(raw_mode) = object.get("storageMode").and_then(Value::as_str) {
        if StorageMode::parse(raw_mode).is_none() {
            return Err(format!(
                "initialize `storageMode` must be `codex` or `project`, got `{raw_mode}`"
            ));
        }
    }
