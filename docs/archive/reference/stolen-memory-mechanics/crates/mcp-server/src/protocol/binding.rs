...
use obsidian_memory_core::StorageMode;
...
pub(super) fn apply_initialize_binding(params: Option<&Value>, state: &mut ServerState) {
    ...
    if let Some(storage_mode) = object
        .get("storageMode")
        .and_then(Value::as_str)
        .and_then(StorageMode::parse)
    {
        state.set_default_storage_mode(storage_mode);
    }
    ...
    match select_root_binding(object) {
        RootSelection::Bound(project_path) => {
            state.bind_project_path(
                project_path,
                state.default_storage_mode(),
                ProjectBindingSource::InitializeRoots,
            );
            return;
        }
        ...
        .and_then(resolve_existing_directory_input)
    {
        state.bind_project_path(
            project_path,
            state.default_storage_mode(),
            ProjectBindingSource::InitializeProjectPath,
        );
    }
}
