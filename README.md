...
Оба launcher'а перед стартом завершают старый `rmu-mcp-server` из этого же checkout, при необходимости пересобирают release binary и только потом запускают новый foreground-процесс. Это закрывает сценарий, когда индекс уже мигрирован новым кодом, а MCP-клиент всё ещё поднимает старый бинарь.
