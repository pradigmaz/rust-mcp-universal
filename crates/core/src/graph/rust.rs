use std::collections::BTreeSet;

use crate::text_utils::symbol_tail;

use super::{
    GraphExtraction, GraphRef, GraphSymbol,
    common::{
        column_from_byte_index, find_identifier_column, iter_call_candidates, iter_path_candidates,
        parse_impl_target, read_identifier, strip_line_comment, strip_rust_item_modifiers,
    },
};

pub(super) fn extract_rust_heuristic(source: &str) -> GraphExtraction {
    let mut symbols = BTreeSet::new();
    let mut deps = BTreeSet::new();
    let mut refs = BTreeSet::new();

    for (line_idx, raw_line) in source.lines().enumerate() {
        let line_no = line_idx + 1;
        let line = strip_line_comment(raw_line);
        let trimmed = line.trim_start();
        if trimmed.is_empty() {
            continue;
        }

        if let Some(symbol) = parse_rust_symbol(line, trimmed, line_no) {
            symbols.insert(symbol);
        }

        if let Some(dep) = parse_rust_dep(trimmed) {
            deps.insert(dep);
        }

        for graph_ref in extract_rust_refs_from_line(line, line_no) {
            refs.insert(graph_ref);
        }
    }

    GraphExtraction {
        symbols: symbols.into_iter().collect(),
        deps: deps.into_iter().collect(),
        refs: refs.into_iter().collect(),
    }
}

fn parse_rust_symbol(line: &str, trimmed: &str, line_no: usize) -> Option<GraphSymbol> {
    let normalized = strip_rust_item_modifiers(trimmed);
    for (keyword, kind) in [
        ("fn ", "function"),
        ("struct ", "struct"),
        ("enum ", "enum"),
        ("trait ", "trait"),
        ("mod ", "module"),
    ] {
        if let Some(rest) = normalized.strip_prefix(keyword)
            && let Some(name) = read_identifier(rest)
        {
            return Some(GraphSymbol {
                column: find_identifier_column(line, &name),
                kind: kind.to_string(),
                line: Some(line_no),
                name,
            });
        }
    }

    if let Some(rest) = normalized.strip_prefix("impl ")
        && let Some(name) = parse_impl_target(rest)
    {
        return Some(GraphSymbol {
            column: find_identifier_column(line, &name),
            kind: "impl".to_string(),
            line: Some(line_no),
            name,
        });
    }

    None
}

fn parse_rust_dep(trimmed: &str) -> Option<String> {
    let normalized = strip_rust_item_modifiers(trimmed);
    let rest = normalized.strip_prefix("use ")?;
    let dep = rest
        .split(';')
        .next()
        .unwrap_or("")
        .trim()
        .split(" as ")
        .next()
        .unwrap_or("")
        .trim();
    if dep.is_empty() {
        None
    } else {
        Some(dep.to_string())
    }
}

fn extract_rust_refs_from_line(line: &str, line_no: usize) -> Vec<GraphRef> {
    let mut out = BTreeSet::new();
    for (candidate, start_idx) in iter_call_candidates(line) {
        let before_candidate = line[..start_idx].trim_end();
        if is_rust_ref_blocked(before_candidate, &candidate) {
            continue;
        }
        out.insert(GraphRef {
            symbol: candidate,
            line: Some(line_no),
            column: Some(column_from_byte_index(line, start_idx)),
        });
    }
    for (candidate, start_idx, end_idx) in iter_path_candidates(line) {
        let before_candidate = line[..start_idx].trim_end();
        let after_candidate = &line[end_idx..];
        if !is_rust_type_ref_candidate(before_candidate, &candidate, after_candidate) {
            continue;
        }
        out.insert(GraphRef {
            symbol: candidate,
            line: Some(line_no),
            column: Some(column_from_byte_index(line, start_idx)),
        });
    }
    out.into_iter().collect()
}

fn is_rust_ref_blocked(before_candidate: &str, candidate: &str) -> bool {
    if candidate.ends_with('!') {
        return true;
    }

    let tail = symbol_tail(candidate);
    if tail.is_empty() {
        return true;
    }
    let Some(first) = tail.chars().next() else {
        return true;
    };
    if !(first.is_ascii_alphabetic() || first == '_') {
        return true;
    }

    if matches!(
        candidate,
        "if" | "match"
            | "while"
            | "for"
            | "loop"
            | "return"
            | "let"
            | "fn"
            | "struct"
            | "enum"
            | "trait"
            | "impl"
            | "mod"
            | "use"
            | "pub"
            | "async"
            | "const"
            | "unsafe"
    ) {
        return true;
    }

    matches!(
        before_candidate.split_whitespace().last().unwrap_or(""),
        "fn" | "if"
            | "match"
            | "while"
            | "for"
            | "loop"
            | "return"
            | "let"
            | "use"
            | "mod"
            | "struct"
            | "enum"
            | "trait"
            | "impl"
    )
}

fn is_rust_type_ref_candidate(
    before_candidate: &str,
    candidate: &str,
    after_candidate: &str,
) -> bool {
    if candidate.contains('.') || candidate.ends_with('!') {
        return false;
    }

    let tail = symbol_tail(candidate);
    let Some(first) = tail.chars().next() else {
        return false;
    };
    if !first.is_ascii_uppercase() {
        return false;
    }

    let last_token = before_candidate.split_whitespace().last().unwrap_or("");
    if matches!(
        last_token,
        "fn" | "struct" | "enum" | "trait" | "mod" | "type"
    ) {
        return false;
    }

    let Some(next_char) = after_candidate.trim_start().chars().next() else {
        return false;
    };
    matches!(next_char, '{' | ',' | ')' | ']' | '}' | ';' | '>' | '=')
}
