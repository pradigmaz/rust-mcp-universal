...
pub(super) fn build_related_hits(
    conn: &Connection,
    normalized_path: &str,
    limit: usize,
) -> Result<Vec<RelatedFileHit>> {
    ...
    let mut hits = by_path
        .into_iter()
        .filter_map(|(candidate_path, acc)| related_hit(candidate_path, acc, base_low_priority))
        .collect::<Vec<_>>();
...
fn related_hit(
    ...
    acc: RelatedAccumulator,
    base_low_priority: bool,
) -> Option<RelatedFileHit> {
    let ref_overlap = acc
    ...
    let mut score =
        acc.dep_overlap as f32 * 0.45 + ref_overlap as f32 * 1.2 + acc.symbol_overlap as f32 * 0.35;
