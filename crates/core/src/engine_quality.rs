    ...
    pub fn rule_violations(&self, options: &RuleViolationsOptions) -> Result<RuleViolationsResult> {
        if !self.db_path.exists() {
            return Ok(RuleViolationsResult {
                summary: empty_rule_violations_summary(),
                hits: Vec::new(),
            });
        }
        ...
        let summary = RuleViolationsSummary {
            ruleset_id: QUALITY_RULESET_ID.to_string(),
            evaluated_files,
            violating_files: filtered.len(),
            total_violations: filtered.iter().map(|hit| hit.violations.len()).sum(),
        };
...
fn empty_rule_violations_summary() -> RuleViolationsSummary {
