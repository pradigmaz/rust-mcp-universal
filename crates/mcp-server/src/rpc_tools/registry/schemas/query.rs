...
pub(crate) fn query_schema(include_semantic_flag: bool) -> Value {
    let mut fields = vec![
        ("query", string_schema("Natural-language query to search for.", Some(1))),
        ("limit", integer_schema("Maximum number of candidates to return.", Some(1))),
    ];
...
pub(crate) fn budget_query_schema() -> Value {
    json_schema_object(
        &[
            ...
            (
                "max_chars",
                integer_schema("Maximum number of characters allowed in the assembled context.", Some(256)),
            ),
            (
                "max_tokens",
                integer_schema("Maximum number of tokens allowed in the assembled context.", Some(64)),
            ),
        ],
        &["query"],
    )
...
pub(crate) fn context_pack_schema() -> Value {
    json_schema_object(
        &[
            ("query", string_schema("Task or question the context pack should support.", Some(1))),
            (
                "mode",
                enum_schema("Context-pack mode tuned to the current task.", &["code", "design", "bugfix"]),
            ),
            ("limit", integer_schema("Maximum number of candidates to consider.", Some(1))),
            (
                "semantic",
...
pub(crate) fn investigation_schema() -> Value {
    json_schema_object(
        &[
            ("seed", string_schema("Concept seed, symbol, path, or path:line probe.", Some(1))),
            (
                "seed_kind",
                enum_schema(
            ...
            ("limit", integer_schema("Maximum number of variants or snippets to return.", Some(1))),
            (
                "auto_index",
                boolean_schema("Automatically build or refresh the index if needed."),
            ),
...
pub(crate) fn query_benchmark_schema() -> Value {
    json_schema_object(
        &[
            ...
            ("k", integer_schema("Top-k cutoff used for retrieval metrics.", Some(1))),
            ("limit", integer_schema("Maximum number of candidates to keep per query.", Some(1))),
            (
                "semantic",
            ...
            (
                "max_chars",
                integer_schema("Maximum number of characters allowed in assembled context payloads.", Some(256)),
            ),
            (
                "max_tokens",
                integer_schema("Maximum number of tokens allowed in assembled context payloads.", Some(64)),
            ),
            (
                "baseline",
                string_schema("Path to a baseline benchmark report for compare mode.", Some(1)),
            ),
            (
                "thresholds",
                string_schema("Path to a thresholds file used for gate enforcement.", Some(1)),
            ),
            ("runs", integer_schema("Number of benchmark repetitions to execute.", Some(1))),
            (
                "enforce_gates",
