...
pub(super) fn search_candidates(args: &Value, state: &mut ServerState) -> Result<Value> {
    ...
    let semantic_fail_mode =
        parse_optional_semantic_fail_mode(args, "search_candidates", "semantic_fail_mode")?
            .unwrap_or(SemanticFailMode::FailOpen);
    let privacy_mode = parse_optional_privacy_mode(args, "search_candidates", "privacy_mode")?
        .unwrap_or(PrivacyMode::Off);
    let vector_layer_enabled =
        parse_optional_bool(args, "search_candidates", "vector_layer_enabled")?.unwrap_or(true);
    let rollout_phase = parse_optional_rollout_phase(args, "search_candidates", "rollout_phase")?
        .unwrap_or(RolloutPhase::Full100);
    let migration_mode =
        parse_optional_migration_mode(args, "search_candidates", "migration_mode")?
